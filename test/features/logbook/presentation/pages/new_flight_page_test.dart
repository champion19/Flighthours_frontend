...
void main() {
  ...
  testWidgets(
    'shows "New Flight" with no route subtitle before typing anything',
    (tester) async {
  ...
  testWidgets(
    'updates the AppBar title with the flight number as the user types',
    (tester) async {
      await tester.pumpWidget(createWidgetUnderTest());
      await tester.pump();
  ...
  testWidgets(
    'lets the pilot pick any origin/destination pair — even one with only '
    'an OACI code and no preconfigured route — and shows the route code '
    'card immediately, with no admin approval step',
    (tester) async {
      // Olaya Herrera only has an OACI code — this used to be unresolvable
      // when matching depended on a preconfigured airline_route.
      ...
      // No route lookup, no pending/approval banner — the pair is usable
      // as soon as both airports are selected.
